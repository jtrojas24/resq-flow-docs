...
The applied local Vector runtime config lives in `fullstack` at:

- `observability/vector/vector.yaml`

This repo also keeps a discoverable example snippet in:
...
- the Rust relay must compile before `make dev` works, so the first run is slower
- `make replay` expects the relay and UI to already be running
- `make replay-direct` is useful when you want to test the UI path without the full relay loop
