...
It may be absent for flow-visible activity that belongs to the flow but does not represent one concrete top-level run, such as polling, scheduler activity, or other pre-work checks.
...
- the child step is `final-result`, so this is also a step log attached to that node
