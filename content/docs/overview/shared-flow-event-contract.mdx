...
- `stage_id` identifies a step attached to that node, so logs that use it are stage logs

That gives us one clean model:

- node logs define the main flow structure
- stage logs show what happened around a node's work

## The key semantic rule

`component_id` is the node identity.

`stage_id` is not a second node identity.
...
- the child step is `final_result`, so this is also a stage log attached to that node
