...
## Run ID implementation pattern

If the flow should support Runs, keep the implementation simple:
...
Use a shared helper plus a flow-specific wrapper.

For example, the current mail flow uses a wrapper around the shared helper so minted IDs look like:

- `mail-pipeline_<uuid-v4>`

Do not re-mint `run_id` at every worker boundary.

The producer should mint once and propagate.
